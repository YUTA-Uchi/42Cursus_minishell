...
/*   Updated: 2025/03/31 15:07:23 by yuuchiya         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	process_heredoc_input(int pipe_write, char *delimiter)
{
	char	*line;
						...
						, t_executor *executor, t_shell_state *shell_state)
{
	...
	status = process_heredoc_input(here_doc_pipe[1], delimiter);
	terminate_shell(executor, shell_state, status);
}
							...
							, t_shell_state *shell_state)
{
	...
	else if (WIFEXITED(status) && WEXITSTATUS(status) != 0)
	{
		if (WEXITSTATUS(status) == 130)
			g_signal = SIGINT;
		if (WEXITSTATUS(status) == E_EOF_ERR)
			shell_state->is_eof = true;
		safe_close(pipe_read);
		return (false);
	}
	return (true);
					...
					, t_shell_state *shell_state)
{
	...
	free(delimiter);
	safe_close(here_doc_pipe[1]);
	if (!wait_heredoc_child(pid, here_doc_pipe[0], shell_state))
		return (false);
